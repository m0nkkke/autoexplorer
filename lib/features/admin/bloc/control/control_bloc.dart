...
class ControlBloc extends Bloc<ControlEvent, ControlState> {
  ...
  final AbstractStorageRepository _storageRepository =
    ...
    on<DeleteUserEvent>((event, emit) async {
      try {
        await _usersRepository
            .deleteUser(event.uid); // Удаляем пользователя из репозитория
  ...
  Future<void> _onLoadUsers(LoadUsers event, Emitter<ControlState> emit) async {
